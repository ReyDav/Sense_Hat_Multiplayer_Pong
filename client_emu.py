...
class Game:
    ...
    def __init__(self):
        self.net = Network()
        self.player = Player(7, 4,(255,255,255))
        self.player2 = Player(0, 4,(255,255,255))
    ...
    def ball_play(self):
        ...
        if ball_position[0] == 1 and self.player2.y-1 <= ball_position[1] <= self.player2.y+1:
            # Reverese the x direction of the ball
            ball_speed[0] = -ball_speed[0]

        # If ball hits player's base
        if ball_position[0] == 7 and self.player.y-1 <= ball_position[1] <= self.player.y+1:
            # Reverese the x direction of the ball
            ball_speed[0] = -ball_speed[0]
        
        # If player didn't catch the ball
        if ball_position[0] == 0:
            # Send message to server to a add point to the other player
